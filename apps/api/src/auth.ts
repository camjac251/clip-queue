 ...
 */
export async function validateTwitchToken(token: string): Promise<{ user_id: string; login: string } | null> {
  // Check cache first
 ...
 */
export function getCacheStats(): { tokenCacheSize: number; roleCacheSize: number } {
  return {
...
export async function checkChannelRole(
  ...
  if (cached) {
    return { isBroadcaster: cached.isBroadcaster, isModerator: cached.isModerator }
  }
...
export function authenticate(
  ...
  validateTwitchToken(token)
    .then((userData) => {
      ...
      return checkChannelRole(clientId, userData.user_id, channelName, token).then((roles) => ({
        ...userData,
        ...roles
      }))
    })
